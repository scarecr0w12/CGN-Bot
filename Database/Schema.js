...
class ValidationError extends Error {
	constructor (errors, document) {
		...
		/* ![{ ERROR }, { ERROR }, ...] */
		else if (!Array.isArray(errors)) throw new SkynetError("GADRIVER_INVALID_PARAMS", { errors });
...
class Definition {
	...
	constructor (raw, key, schema) {
		...
		if (raw[MapSymbol]) {
			...
			if (!this.type) throw new SkynetError("GADRIVER_ERROR", {}, "Your schema is not configured correctly.");
...
class Schema {
	...
	constructor (schema, options = {}) {
		...
		if (this._options.map && !this._definitions.has("_id")) throw new SkynetError("GADRIVER_ERROR", {}, "A Map type Schema must have an _id value.");
	}
