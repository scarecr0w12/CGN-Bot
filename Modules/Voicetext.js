...
module.exports = {
	addMember: async (guild, voiceChannel, member) => {
		...
		if (!channel || channel.type !== "text") {
			channel = await guild.channels.create(`${voiceChannel.name.replaceAll(" ", "").toLowerCase().replace(/[^-_a-z0-9]/ig, "")}-voicetext`, {
				...
				permissionOverwrites: [{ id: guild.id, deny: PermissionFlagsBits.ViewChannel, type: "role" }],
	...
	removeMember: async (guild, voiceChannel, member) => {
		const channel = guild.channels.cache.find(ch => ch.topic && ch.topic.includes(`gab:voicetext:${voiceChannel.id}`));
