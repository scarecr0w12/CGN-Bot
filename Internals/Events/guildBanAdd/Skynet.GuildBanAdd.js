...
class GuildBanAdd extends BaseEvent {
	async handle (guild, user) {
		...
		if (serverDocument.config.moderation.isEnabled && serverDocument.config.moderation.status_messages.member_banned_message.isEnabled) {
			logger.verbose(`Member '${user.username}' banned from guild '${guild}'.`, { svrid: guild.id, usrid: user.id });
			const channel = guild.channels.cache.get(serverDocument.config.moderation.status_messages.member_banned_message.channel_id);
			if (channel) {
				...
				if (!channelDocument || channelDocument.bot_enabled) {
					...
					if (!message) return;
					channel.send({
						embeds: [StatusMessages.GUILD_BAN_ADD(message, user)],
					}).catch(err => {
