...
module.exports = async ({ client, Constants: { Colors } }, { serverDocument, memberDocument }, msg, commandData) => {
	...
	const sortedMembers = Object.values(serverDocument.members).filter(m => m.messages && msg.guild.members.cache.has(m._id)).sort((a, b) => b.messages - a.messages);
	const totalMessages = sortedMembers.reduce((a, b) => (a.messages || a) + b.messages, 0);
	const description = sortedMembers
		.slice(0, 8)
		.map(mDoc => {
			const member = msg.guild.members.cache.get(mDoc._id);
			return [
