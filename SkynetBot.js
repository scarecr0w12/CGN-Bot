...
const { Encryption, Utils, getGuild, PostTotalData, Traffic, Trivia, Polls, Giveaways, Lotteries, SkynetClient, Temp: TemporaryStorage } = require("./Modules/index");
...
const { configJS, configJSON, auth } = loadConfigs();
...
Boot({ configJS, configJSON, auth }, scope).then(async () => {
	...
	client.IPC.on("getGuild", async (msg, callback) => {
		if (msg.target === "*") {
			...
			let guilds = msg.settings.mutualOnlyTo ? client.guilds.cache.filter(guild => guild.members.cache.has(msg.settings.mutualOnlyTo)) : client.guilds.cache;
	...
	client.IPC.on("awaitMessage", async (msg, callback) => {
		...
		let channel = client.channels.cache.get(msg.ch);
